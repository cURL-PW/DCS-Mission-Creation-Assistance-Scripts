...
function SuppressedUnit(id,Suppresse_rate,MinOffDelay,MaxOffDelay,MinStartDelay,MaxStartDelay)
...
function SuppressedGroup(id,Suppresse_rate,MinOffDelay,MaxOffDelay,MinStartDelay,MaxStartDelay)
    local _evade = mist.random (1,100) -- random number for chance of evading action
    if (_evade <= Suppresse_rate) then --ここで何割起動するか設定する
        local RadarOffDelay = math.random(MinOffDelay, MaxOffDelay) --ミサイル発射時から何秒後に停波するか決める
        local RadarOnDelay = math.random(MinStartDelay, MaxStartDelay) --停波後から何秒後に起動するか決める
        if SuppressedGroups[id.groupName] == nil then
            ...
            timer.scheduleFunction(SuppressionGroupStart, id, SuppressedGroups[id.groupName].SuppressionStartTime)	--Schedule the SuppressionStart() function
            timer.scheduleFunction(SuppressionGroupEnd, id, SuppressedGroups[id.groupName].SuppressionEndTime)	--Schedule the SuppressionEnd() function
            if debugmode == true then trigger.action.outText( string.format("Radar Off " ..string.format(RadarOffDelay)), 20) end --debug message
            if debugmode == true then trigger.action.outText( string.format("Radar On " ..string.format(RadarOnDelay)), 20) end --debug message
        end
    end
...
function SuppressionGroupEnd(id)
    id.groupName:getController():setOption(AI.Option.Ground.id.ALARM_STATE,AI.Option.Ground.val.ALARM_STATE.RED)
    SuppressedGroups[id.groupName] = nil
end

function SuppressionGroupStart(id)
    id.groupName:getController():setOption(AI.Option.Ground.id.ALARM_STATE,AI.Option.Ground.val.ALARM_STATE.GREEN)
end
